...
let openFullImage = document.querySelector('.popup__image-full');      //выбираем нашу картинку
let popoupDescription = document.querySelector('.popup__description');  //подпись к карточки
let openPopoupFullImage = document.querySelector('.popup_full-card'); //открытие окна на полной карточке 
...
function createCardTemplate(name, link) {
   ...
   cardElement.querySelector('.element__photo').addEventListener('click', function() {               //передаем данные в параметры ф-ии
      openFullImage.src = link;
      popoupDescription.textContent = name;
      openPopup(openPopoupFullImage);
   })
...
openFullImage.addEventListener('click', function() {
   openPopup(openPopoupFullImage);
})

closePopupFullImage.addEventListener('click', function() {
   closePopup(openPopoupFullImage);
})
